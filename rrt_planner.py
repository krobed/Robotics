...
class RRTPlanner:
    ...
    def sample_random_position(self):
        ...
        sampled_x = np.random.randint(self._map_width)
        sampled_y = np.random.randint(self._map_height)
    ...
    def check_for_collision(self, new_position):
        ...
        for i in [-2, 0, 2]:
            for j in [-2, 0, 2]:
                if self._map[np.clip(int(new_position[1]) + j, 0, self._map_height - 1),
                            np.clip(int(new_position[0]) + i, 0, self._map_width - 1)] == 1:
